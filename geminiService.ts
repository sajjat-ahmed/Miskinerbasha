...
// Initialize the GoogleGenAI client using the environment variable as per guidelines.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

export const getSmartRoomDescription = async (details: any) => {
  try {
...
export const getAreaInsights = async (area: string) => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
